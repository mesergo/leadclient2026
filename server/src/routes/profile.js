...
router.patch('/', asyncHandler(async (req, res) => {
  const { first_name, last_name, display_name, language } = req.body || {};
  await query('UPDATE users SET first_name = COALESCE(?, first_name), last_name = COALESCE(?, last_name), display_name = COALESCE(?, display_name), language = COALESCE(?, language) WHERE id = ?',
    [first_name ?? null, last_name ?? null, display_name ?? null, language ?? null, req.user.id]);
  res.json({ ok: true });
}));
