...
export const api = {
  ...
  billingPrices: (token) => request('/api/billing/prices', { token }),
  createPackage: (body, token) => request('/api/billing/packages', { method: 'POST', body, token }),
  updatePackage: (id, body, token) => request(`/api/billing/packages/${id}`, { method: 'PATCH', body, token }),
  deletePackage: (id, token) => request(`/api/billing/packages/${id}`, { method: 'DELETE', token }),
