...
export default function CompaniesPage() {
  ...
  async function impersonate(c) { if (!confirm(`להתחבר כמנהל "${c.name}"?`)) return; try { const d = await api.impersonateCompany(c.id, token); setImpersonatedSession(d.token, d.user); location.href = '/'; } catch (e) { setError(e.message); } }

  return (
    <div>
      <div className="page-header"><h1>חברות</h1></div>
      {error && <p className="error">{error}</p>}
      {(user?.role === 'super_admin' || user?.role === 'agency_admin') && (
        <form className="inline-form" onSubmit={create}>
          <input placeholder="שם חברה חדשה" value={name} onChange={(e) => setName(e.target.value)} />
          <button className="btn btn-primary">הוספת חברה</button>
        </form>
      )}
      <div className="card-grid">
        {rows.map((c) => (
          <div className="entity-card" key={c.id}>
            <div className="entity-card-head">
              ...
              <h3>{c.name}</h3>
              <span className={'status-pill ' + (c.is_active ? 'active' : 'inactive')}>{c.is_active ? 'פעילה' : 'מושעית'}</span>
            </div>
            <dl className="entity-card-stats"><div><dt>סוכנות</dt><dd style={{ fontSize: 12 }}>{c.agency_name || '-'}</dd></div><div><dt>ערוצים</dt><dd>{c.services_count}</dd></div></dl>
        ...
        {rows.length === 0 && <p className="muted">אין חברות.</p>}
      </div>
    </div>
  );
}
