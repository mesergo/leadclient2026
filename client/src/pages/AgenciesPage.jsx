...
export default function AgenciesPage() {
  ...
  return (
    <div>
      <div className="page-header"><h1>סוכנויות</h1></div>
      {error && <p className="error">{error}</p>}
      <form className="inline-form" onSubmit={create}>
        <input placeholder="שם סוכנות חדשה" value={name} onChange={(e) => setName(e.target.value)} />
        <button className="btn btn-primary">הוספת סוכנות</button>
      </form>
      <div className="card-grid">
        {rows.map((a) => (
          <div className="entity-card" key={a.id}>
            <div className="entity-card-head">
              ...
              <span className={'status-pill ' + (a.is_active ? 'active' : 'inactive')}>{a.is_active ? 'פעילה' : 'מושעית'}</span>
            </div>
            <dl className="entity-card-stats"><div><dt>חברות</dt><dd>{a.companies_count}</dd></div><div><dt>IVR</dt><dd style={{ fontSize: 12 }}>{a.ivr_provider}</dd></div></dl>
        ...
        {rows.length === 0 && <p className="muted">אין סוכנויות.</p>}
      </div>
    </div>
  );
}
